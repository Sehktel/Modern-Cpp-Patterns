...
// ============================================================================
// РЈРЇР—Р’РРњР«Р• Р Р•РђР›РР—РђР¦РР smart_pointers Р”Р›РЇ РђРќРђР›РР—Рђ Р‘Р•Р—РћРџРђРЎРќРћРЎРўР
// ============================================================================
...
public:
    Vulnerablesmart_pointers() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РєРѕРЅСЃС‚СЂСѓРєС‚РѕСЂ СЃ СѓСЏР·РІРёРјРѕСЃС‚СЏРјРё
        std::cout << "Vulnerablesmart_pointers СЃРѕР·РґР°РЅ" << std::endl;
    }
    ...
    ~Vulnerablesmart_pointers() {
        ...
        std::cout << "Vulnerablesmart_pointers СѓРЅРёС‡С‚РѕР¶РµРЅ" << std::endl;
    }
};
...
// ----------------------------------------------------------------------------
class AnotherVulnerablesmart_pointers {
private:
    // TODO: Р”РѕР±Р°РІРёС‚СЊ РїСЂРёРІР°С‚РЅС‹Рµ С‡Р»РµРЅС‹ РєР»Р°СЃСЃР°
    
public:
    AnotherVulnerablesmart_pointers() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РєРѕРЅСЃС‚СЂСѓРєС‚РѕСЂ СЃ СѓСЏР·РІРёРјРѕСЃС‚СЏРјРё
        std::cout << "AnotherVulnerablesmart_pointers СЃРѕР·РґР°РЅ" << std::endl;
    }
    ...
    ~AnotherVulnerablesmart_pointers() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РґРµСЃС‚СЂСѓРєС‚РѕСЂ
        std::cout << "AnotherVulnerablesmart_pointers СѓРЅРёС‡С‚РѕР¶РµРЅ" << std::endl;
    }
};
...
void runStaticAnalysis() {
    ...
    std::cout << "   clang --analyze smart_pointers_vulnerabilities.cpp" << std::endl;
    std::cout << "2. Cppcheck:" << std::endl;
    std::cout << "   cppcheck --enable=all smart_pointers_vulnerabilities.cpp" << std::endl;
...
void runDynamicAnalysis() {
    ...
    std::cout << "   g++ -fsanitize=address -g smart_pointers_vulnerabilities.cpp -o smart_pointers_asan" << std::endl;
    std::cout << "   ./smart_pointers_asan" << std::endl;
    std::cout << "2. ThreadSanitizer:" << std::endl;
    std::cout << "   g++ -fsanitize=thread -g smart_pointers_vulnerabilities.cpp -o smart_pointers_tsan" << std::endl;
    std::cout << "   ./smart_pointers_tsan" << std::endl;
    std::cout << "3. Valgrind:" << std::endl;
    std::cout << "   valgrind --tool=memcheck ./smart_pointers_vulnerabilities" << std::endl;
}
...
int main() {
    std::cout << "=== РђРќРђР›РР— РЈРЇР—Р’РРњРћРЎРўР•Р™ Р’ РџРђРўРўР•Р РќР• smart_pointers ===" << std::endl;
