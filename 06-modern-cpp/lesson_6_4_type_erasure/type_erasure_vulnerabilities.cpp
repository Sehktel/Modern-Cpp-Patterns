...
// ============================================================================
// РЈРЇР—Р’РРњР«Р• Р Р•РђР›РР—РђР¦РР type_erasure Р”Р›РЇ РђРќРђР›РР—Рђ Р‘Р•Р—РћРџРђРЎРќРћРЎРўР
// ============================================================================
...
public:
    Vulnerabletype_erasure() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РєРѕРЅСЃС‚СЂСѓРєС‚РѕСЂ СЃ СѓСЏР·РІРёРјРѕСЃС‚СЏРјРё
        std::cout << "Vulnerabletype_erasure СЃРѕР·РґР°РЅ" << std::endl;
    }
    ...
    ~Vulnerabletype_erasure() {
        ...
        std::cout << "Vulnerabletype_erasure СѓРЅРёС‡С‚РѕР¶РµРЅ" << std::endl;
    }
};
...
// ----------------------------------------------------------------------------
class AnotherVulnerabletype_erasure {
private:
    // TODO: Р”РѕР±Р°РІРёС‚СЊ РїСЂРёРІР°С‚РЅС‹Рµ С‡Р»РµРЅС‹ РєР»Р°СЃСЃР°
    
public:
    AnotherVulnerabletype_erasure() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РєРѕРЅСЃС‚СЂСѓРєС‚РѕСЂ СЃ СѓСЏР·РІРёРјРѕСЃС‚СЏРјРё
        std::cout << "AnotherVulnerabletype_erasure СЃРѕР·РґР°РЅ" << std::endl;
    }
    ...
    ~AnotherVulnerabletype_erasure() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РґРµСЃС‚СЂСѓРєС‚РѕСЂ
        std::cout << "AnotherVulnerabletype_erasure СѓРЅРёС‡С‚РѕР¶РµРЅ" << std::endl;
    }
};
...
void runStaticAnalysis() {
    ...
    std::cout << "   clang --analyze type_erasure_vulnerabilities.cpp" << std::endl;
    std::cout << "2. Cppcheck:" << std::endl;
    std::cout << "   cppcheck --enable=all type_erasure_vulnerabilities.cpp" << std::endl;
...
void runDynamicAnalysis() {
    ...
    std::cout << "   g++ -fsanitize=address -g type_erasure_vulnerabilities.cpp -o type_erasure_asan" << std::endl;
    std::cout << "   ./type_erasure_asan" << std::endl;
    std::cout << "2. ThreadSanitizer:" << std::endl;
    std::cout << "   g++ -fsanitize=thread -g type_erasure_vulnerabilities.cpp -o type_erasure_tsan" << std::endl;
    std::cout << "   ./type_erasure_tsan" << std::endl;
    std::cout << "3. Valgrind:" << std::endl;
    std::cout << "   valgrind --tool=memcheck ./type_erasure_vulnerabilities" << std::endl;
}
...
int main() {
    std::cout << "=== РђРќРђР›РР— РЈРЇР—Р’РРњРћРЎРўР•Р™ Р’ РџРђРўРўР•Р РќР• type_erasure ===" << std::endl;
