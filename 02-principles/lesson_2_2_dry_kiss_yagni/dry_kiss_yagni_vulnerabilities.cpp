...
// ============================================================================
// РЈРЇР—Р’РРњР«Р• Р Р•РђР›РР—РђР¦РР dry_kiss_yagni Р”Р›РЇ РђРќРђР›РР—Рђ Р‘Р•Р—РћРџРђРЎРќРћРЎРўР
// ============================================================================
...
public:
    Vulnerabledry_kiss_yagni() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РєРѕРЅСЃС‚СЂСѓРєС‚РѕСЂ СЃ СѓСЏР·РІРёРјРѕСЃС‚СЏРјРё
        std::cout << "Vulnerabledry_kiss_yagni СЃРѕР·РґР°РЅ" << std::endl;
    }
    ...
    ~Vulnerabledry_kiss_yagni() {
        ...
        std::cout << "Vulnerabledry_kiss_yagni СѓРЅРёС‡С‚РѕР¶РµРЅ" << std::endl;
    }
};
...
// ----------------------------------------------------------------------------
class AnotherVulnerabledry_kiss_yagni {
private:
    // TODO: Р”РѕР±Р°РІРёС‚СЊ РїСЂРёРІР°С‚РЅС‹Рµ С‡Р»РµРЅС‹ РєР»Р°СЃСЃР°
    
public:
    AnotherVulnerabledry_kiss_yagni() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РєРѕРЅСЃС‚СЂСѓРєС‚РѕСЂ СЃ СѓСЏР·РІРёРјРѕСЃС‚СЏРјРё
        std::cout << "AnotherVulnerabledry_kiss_yagni СЃРѕР·РґР°РЅ" << std::endl;
    }
    ...
    ~AnotherVulnerabledry_kiss_yagni() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РґРµСЃС‚СЂСѓРєС‚РѕСЂ
        std::cout << "AnotherVulnerabledry_kiss_yagni СѓРЅРёС‡С‚РѕР¶РµРЅ" << std::endl;
    }
};
...
void runStaticAnalysis() {
    ...
    std::cout << "   clang --analyze dry_kiss_yagni_vulnerabilities.cpp" << std::endl;
    std::cout << "2. Cppcheck:" << std::endl;
    std::cout << "   cppcheck --enable=all dry_kiss_yagni_vulnerabilities.cpp" << std::endl;
...
void runDynamicAnalysis() {
    ...
    std::cout << "   g++ -fsanitize=address -g dry_kiss_yagni_vulnerabilities.cpp -o dry_kiss_yagni_asan" << std::endl;
    std::cout << "   ./dry_kiss_yagni_asan" << std::endl;
    std::cout << "2. ThreadSanitizer:" << std::endl;
    std::cout << "   g++ -fsanitize=thread -g dry_kiss_yagni_vulnerabilities.cpp -o dry_kiss_yagni_tsan" << std::endl;
    std::cout << "   ./dry_kiss_yagni_tsan" << std::endl;
    std::cout << "3. Valgrind:" << std::endl;
    std::cout << "   valgrind --tool=memcheck ./dry_kiss_yagni_vulnerabilities" << std::endl;
}
...
int main() {
    std::cout << "=== РђРќРђР›РР— РЈРЇР—Р’РРњРћРЎРўР•Р™ Р’ РџРђРўРўР•Р РќР• dry_kiss_yagni ===" << std::endl;
