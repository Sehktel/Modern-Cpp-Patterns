...
// ============================================================================
// РЈРЇР—Р’РРњР«Р• Р Р•РђР›РР—РђР¦РР producer_consumer Р”Р›РЇ РђРќРђР›РР—Рђ Р‘Р•Р—РћРџРђРЎРќРћРЎРўР
// ============================================================================
...
public:
    ...
    ~Vulnerableproducer_consumer() {
        ...
        std::cout << "Vulnerableproducer_consumer СѓРЅРёС‡С‚РѕР¶РµРЅ" << std::endl;
    }
};
...
// ----------------------------------------------------------------------------
class AnotherVulnerableproducer_consumer {
private:
    // TODO: Р”РѕР±Р°РІРёС‚СЊ РїСЂРёРІР°С‚РЅС‹Рµ С‡Р»РµРЅС‹ РєР»Р°СЃСЃР°
    
public:
    AnotherVulnerableproducer_consumer() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РєРѕРЅСЃС‚СЂСѓРєС‚РѕСЂ СЃ СѓСЏР·РІРёРјРѕСЃС‚СЏРјРё
        std::cout << "AnotherVulnerableproducer_consumer СЃРѕР·РґР°РЅ" << std::endl;
    }
    ...
    ~AnotherVulnerableproducer_consumer() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РґРµСЃС‚СЂСѓРєС‚РѕСЂ
        std::cout << "AnotherVulnerableproducer_consumer СѓРЅРёС‡С‚РѕР¶РµРЅ" << std::endl;
    }
};
...
void runStaticAnalysis() {
    ...
    std::cout << "   clang --analyze producer_consumer_vulnerabilities.cpp" << std::endl;
    std::cout << "2. Cppcheck:" << std::endl;
    std::cout << "   cppcheck --enable=all producer_consumer_vulnerabilities.cpp" << std::endl;
...
void runDynamicAnalysis() {
    ...
    std::cout << "   g++ -fsanitize=address -g producer_consumer_vulnerabilities.cpp -o producer_consumer_asan" << std::endl;
    std::cout << "   ./producer_consumer_asan" << std::endl;
    std::cout << "2. ThreadSanitizer:" << std::endl;
    std::cout << "   g++ -fsanitize=thread -g producer_consumer_vulnerabilities.cpp -o producer_consumer_tsan" << std::endl;
    std::cout << "   ./producer_consumer_tsan" << std::endl;
    std::cout << "3. Valgrind:" << std::endl;
    std::cout << "   valgrind --tool=memcheck ./producer_consumer_vulnerabilities" << std::endl;
}
...
int main() {
    std::cout << "=== РђРќРђР›РР— РЈРЇР—Р’РРњРћРЎРўР•Р™ Р’ РџРђРўРўР•Р РќР• producer_consumer ===" << std::endl;
