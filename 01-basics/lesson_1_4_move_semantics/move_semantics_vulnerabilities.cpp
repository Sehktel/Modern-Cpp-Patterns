...
// ============================================================================
// РЈРЇР—Р’РРњР«Р• Р Р•РђР›РР—РђР¦РР move_semantics Р”Р›РЇ РђРќРђР›РР—Рђ Р‘Р•Р—РћРџРђРЎРќРћРЎРўР
// ============================================================================
...
public:
    Vulnerablemove_semantics() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РєРѕРЅСЃС‚СЂСѓРєС‚РѕСЂ СЃ СѓСЏР·РІРёРјРѕСЃС‚СЏРјРё
        std::cout << "Vulnerablemove_semantics СЃРѕР·РґР°РЅ" << std::endl;
    }
    ...
    ~Vulnerablemove_semantics() {
        ...
        std::cout << "Vulnerablemove_semantics СѓРЅРёС‡С‚РѕР¶РµРЅ" << std::endl;
    }
};
...
// ----------------------------------------------------------------------------
class AnotherVulnerablemove_semantics {
private:
    // TODO: Р”РѕР±Р°РІРёС‚СЊ РїСЂРёРІР°С‚РЅС‹Рµ С‡Р»РµРЅС‹ РєР»Р°СЃСЃР°
    
public:
    AnotherVulnerablemove_semantics() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РєРѕРЅСЃС‚СЂСѓРєС‚РѕСЂ СЃ СѓСЏР·РІРёРјРѕСЃС‚СЏРјРё
        std::cout << "AnotherVulnerablemove_semantics СЃРѕР·РґР°РЅ" << std::endl;
    }
    ...
    ~AnotherVulnerablemove_semantics() {
        // TODO: Р”РѕР±Р°РІРёС‚СЊ РґРµСЃС‚СЂСѓРєС‚РѕСЂ
        std::cout << "AnotherVulnerablemove_semantics СѓРЅРёС‡С‚РѕР¶РµРЅ" << std::endl;
    }
};
...
void runStaticAnalysis() {
    ...
    std::cout << "   clang --analyze move_semantics_vulnerabilities.cpp" << std::endl;
    std::cout << "2. Cppcheck:" << std::endl;
    std::cout << "   cppcheck --enable=all move_semantics_vulnerabilities.cpp" << std::endl;
...
void runDynamicAnalysis() {
    ...
    std::cout << "   g++ -fsanitize=address -g move_semantics_vulnerabilities.cpp -o move_semantics_asan" << std::endl;
    std::cout << "   ./move_semantics_asan" << std::endl;
    std::cout << "2. ThreadSanitizer:" << std::endl;
    std::cout << "   g++ -fsanitize=thread -g move_semantics_vulnerabilities.cpp -o move_semantics_tsan" << std::endl;
    std::cout << "   ./move_semantics_tsan" << std::endl;
    std::cout << "3. Valgrind:" << std::endl;
    std::cout << "   valgrind --tool=memcheck ./move_semantics_vulnerabilities" << std::endl;
}
...
int main() {
    std::cout << "=== РђРќРђР›РР— РЈРЇР—Р’РРњРћРЎРўР•Р™ Р’ РџРђРўРўР•Р РќР• move_semantics ===" << std::endl;
